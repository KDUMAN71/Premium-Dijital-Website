...
const S = StyleSheet.create({
  ...
  // Mavi başlık şeridi — teklif dosyası stili
  pageHeaderBand: {
  ...
  // Sayfa başlık bölümü (şeridin altı)
  pageContentHeader: {
    paddingHorizontal: 40,
    paddingTop: 22,
    paddingBottom: 0,
  ...
  return (
    <View style={S.pageFooter} fixed>
      <View style={S.footerColLeft}>
        ...
        <Text style={S.footerText}>premiumdijital.com</Text>
      </View>
      <View style={S.footerColCenter}>
        <Text style={[S.footerText, { textAlign: "center" }]}>
          Premium Dijital Reklam Ajansi ve Pazarlama
        </Text>
        <Text style={[S.footerText, { textAlign: "center" }]}>
          Ziya Gokalp Mah. Mall Of Istanbul The Office No:7E D:136, 34490 Baskasehir/Istanbul
        </Text>
      </View>
      <View style={S.footerColRight}>
        <Text style={S.footerTextAccent}>@premiumdijital</Text>
        <Text style={S.footerTextAccent}>@premiumdijital</Text>
        <Text style={[S.footerText, { textAlign: "right" }]}>{label}</Text>
      </View>
    </View>
  );
}
    ...
    <Page size="A4" style={S.coverPage}>
      ...
      <View style={S.coverSidebar}>
        <View style={S.coverSidebarTop}>
          <Text style={S.coverSidebarLabel}>{"A\nN\nA\nL\nI\nZ"}</Text>
        </View>
      ...
      <View style={S.coverContent}>
        ...
        <View>
          ...
          <Text style={S.coverMainTitle}>ANALIZI</Text>
          ...
          >
            ...
            >
              <Text
                style={{ fontSize: 18, fontWeight: 700, color: COLORS.white, lineHeight: 1 }}
              >
                {data.overallScore}
              </Text>
            </View>
            <View>
              <Text
                style={{ fontSize: 7, color: COLORS.gray, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 3 }}
              >
                Genel Dijital Skor
              </Text>
              <Text style={{ fontSize: 15, fontWeight: 700, color }}>{data.scoreLabel}</Text>
            </View>
          </View>
        ...
        <View>
          ...
          <Text style={S.coverFooterMeta}>
            DOSYA NO: {data.reportId} · GIZLIDIR
          </Text>
        </View>
      </View>
    </Page>
...
function ExecutiveSummaryPage({ data }: { data: AnalysisData }) {
  ...
  return (
    <Page size="A4" style={S.innerPage}>
      <PageHeader
        pageNum="02"
        category="YONETICI OZETI"
        clientName={data.clientName}
        title="Pazar Durusu ve Risk Analizi"
      />
      <View style={S.body}>
          ...
          {categories.map((cat) => {
            ...
            return (
              <View key={cat.name} style={S.categoryCard}>
                <Text style={[S.categoryScoreVal, { color: c }]}>
...
function SeoPage({ data }: { data: AnalysisData }) {
  ...
  return (
    <Page size="A4" style={S.innerPage}>
      <PageHeader
        pageNum="03"
        category="SEO & WEB MIMARISI"
        clientName={data.clientName}
        title="Teknik Altyapi ve Gorunurluk Analizi"
      />
      <View style={S.body}>
        <ScoreAndMetrics
          score={seo.score}
  ...
  return (
    <Page size="A4" style={S.innerPage}>
      <PageHeader
        ...
        clientName={data.clientName}
        title="Reklam Harcamasi ve Donusum Analizi"
      />
      <View style={S.body}>
        <ScoreAndMetrics
          score={ppc.score}
          metrics={[
            {
              value: ppc.qualityScore !== null ? `${ppc.qualityScore}/10` : "—",
              label: "Kalite Skoru",
              color: ppc.qualityScore !== null && ppc.qualityScore < 5 ? COLORS.danger : COLORS.success,
            },
...
function SocialPage({ data }: { data: AnalysisData }) {
  ...
  return (
    <Page size="A4" style={S.innerPage}>
      <PageHeader
        pageNum="05"
        category="SOSYAL MEDYA & ICERIK"
        clientName={data.clientName}
        title="Marka Tutarliligi ve Etkilesim Analizi"
      />
      <View style={S.body}>
        <ScoreAndMetrics
          score={social.score}
          metrics={[
            ...
            {
              value: social.consistencyScore !== null ? social.consistencyScore : "—",
              label: "Tutarlilik Skoru",
              color: COLORS.purple,
            },
          ]}
        />
...
function OperationsPage({ data }: { data: AnalysisData }) {
  ...
  return (
    <Page size="A4" style={S.innerPage}>
      <PageHeader
        pageNum="06"
        category="DIJITAL OPERASYON"
        clientName={data.clientName}
        title="Sistem Olgunlugu ve Otomasyon Bosluklar"
      />
      <View style={S.body}>
        {/* Skor + tech stack yan yana */}
        <View style={{ flexDirection: "row", gap: 16, marginBottom: 18 }}>
          <View
            style={[
              S.scoreCircle,
              { backgroundColor: scoreColor(operations.score) },
            ]}
          >
          ...
          <View style={{ flex: 1 }}>
            <Text style={[S.listTitle, { marginBottom: 6 }]}>Tech Stack</Text>
            ...
            <Text style={[S.listTitle, { marginBottom: 6, marginTop: 8 }]}>
              Otomasyon Bosluklari
            </Text>
        ...
        <View
          style={{ height: 0.5, backgroundColor: COLORS.border, marginBottom: 14 }}
        />
        ...
        <View style={S.ctaBox}>
          ...
          <Text style={S.ctaSubtext}>
            Bu raporda tespit edilen eksiklikler icin ozel bir buyume plani
            hazirliyoruz. Ilk strateji gorusmesi ucretsiz ve baglayici degildir.
          </Text>
...
export default function ClientReport({ data }: { data: AnalysisData }) {
  return (
    <Document
      ...
      subject="Dijital Varlik ve Konumlandirma Analizi"
    >
      <CoverPage data={data} />
