...
async function processAdsScorecard(data: AdsScorecardValues) {
  try {
    const res = await fetch("/api/ads-scorecard", {
      method: "POST",
...
export default function LeadScorecard() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<AdsScorecardValues>({
    resolver: zodResolver(adsScorecardSchema),
    mode: "onTouched",
  });

  const onSubmit = (data: AdsScorecardValues) => {
    setServerError(null);
    startTransition(async () => {
      const response = await processAdsScorecard(data);
      if (response.error) {
    ...
    >
        ...
        <div className="mb-20 text-center max-w-3xl mx-auto">
          <ScrollReveal>
            <span className="inline-block px-4 py-1 rounded-full border border-white/10 bg-white/5 text-white/50 text-[10px] font-bold uppercase tracking-[0.2em] mb-4">
              Google Ads Performans Analizi
            </span>
            <h2 className="text-3xl md:text-5xl font-black tracking-tighter mb-6">
              Google Ads{" "}
              <span className="text-brand-blue">Performans Skorunuzu</span>{" "}
              Ölçün
            </h2>
            <p className="text-gray-400 text-lg leading-relaxed italic">
              Kampanya bütçenizi, tıklama maliyetinizi ve dönüşümlerinizi girin;
              anında performans skorunuzu görün ve optimize edebileceğiniz
              alanları keşfedin.
            </p>
          </ScrollReveal>
        </div>
            ...
            {!result ? (
              ...
              >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        className={cn(
                          "input-element",
                          errors.cpc && "border-red-500/50",
                        )}
                      />
                      {errors.cpc && (
                        <p className="error-text">{errors.cpc.message}</p>
                      )}
                    </div>
                  </div>
                </fieldset>
                ...
                >
                  ...
                  <span className="relative z-10 flex items-center gap-3">
                    ...
                    Performans Skorunu Hesapla ve Hemen Rapor Al
                  </span>
                </motion.button>
              </motion.form>
            ) : (
              ...
              >
                <div className="inline-flex items-center justify-center relative">
                  <div className="absolute inset-0 bg-brand-blue/10 blur-[60px] rounded-full" />
                ...
                <div className="space-y-3 max-w-md mx-auto">
                  <h3 className="text-2xl font-bold tracking-tight">
                    Hesap Performans Skorunuz
                  </h3>
                  <p className="text-gray-400 text-sm leading-relaxed">
                    {result.message} <br />
                    Bu skor, reklam bütçenizi ve teklif stratejinizi optimize
                    etmeniz için bir yol göstericidir. Daha yüksek dönüşüm için
                    analiz görüşmesi planlayabilirsiniz.
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                  ...
                  <a
                    href={`https://wa.me/905425658010?text=Merhaba%2C%20Google%20Ads%20performans%20skorum%20${result.score}%20%C3%A7%C4%B1kt%C4%B1.%20%C3%9Ccretsiz%20strateji%20g%C3%B6r%C3%BC%C5%9Fmesi%20talep%20ediyorum.`}
                    target="_blank"
      ...
      <style jsx>{`
        ...
        .input-element:focus {
          ...
          box-shadow: 0 0 15px rgba(0, 0, 200, 0.15);
        }
        .input-label {
          display: block;
          font-size: 10px;
