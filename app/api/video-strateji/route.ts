...
export async function POST(req: NextRequest) {
  try {
    ...
    const { sektor, hedef, butce, fullName, email, website, company_honey } = body;

    // Honeypot
    if (company_honey) return NextResponse.json({ error: "Invalid" }, { status: 400 });

    // Temel validasyon
    if (!sektor || !hedef || !butce || !fullName || !email) {
    ...
    const yoneticiResult = await resend.emails.send({
      ...
      subject: `[Video Strateji] ${fullName} — ${sektor} / ${hedef}`,
      html: `
        <h2>Yeni Video Strateji Talebi</h2>
        <p><strong>Ad Soyad:</strong> ${fullName}</p>
        <p><strong>E-posta:</strong> ${email}</p>
        <p><strong>Web Sitesi:</strong> ${website || "—"}</p>
        <hr/>
        <p><strong>Sektör:</strong> ${sektor}</p>
        <p><strong>Kampanya Hedefi:</strong> ${hedef}</p>
        <p><strong>Aylık Bütçe:</strong> ${butce}</p>
        <hr/>
        <p style="color:#999;font-size:12px;">premium-digital-web · video-strateji form</p>
      `,
    });
    console.log("[video-strateji] yönetici:", JSON.stringify(yoneticiResult));
    ...
    const kullaniciResult = await resend.emails.send({
      ...
      subject: "Video Strateji Öneriniz Yolda | Premium Dijital",
      html: `
        <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#050505;color:#fff;padding:40px;border-radius:16px;">
          ...
          <p style="color:#aaa;font-size:15px;margin-bottom:32px;">
            Talebinizi aldık. ${sektor} sektörü için ${hedef} odaklı video strateji önerinizi hazırlıyoruz.
          </p>
          <div style="background:#111;border:1px solid #333;border-radius:12px;padding:24px;margin-bottom:32px;">
            ...
            <p style="color:#fff;font-size:14px;margin:4px 0;"><strong>Sektör:</strong> ${sektor}</p>
            <p style="color:#fff;font-size:14px;margin:4px 0;"><strong>Hedef:</strong> ${hedef}</p>
          ...
          <p style="color:#ccc;font-size:14px;line-height:1.7;margin-bottom:24px;">
            Sektörünüze ve hedeflediğiniz aksiyona özel video formatı önerileri, platform seçimi ve
            tahmini bütçe dağılımını içeren strateji dokümanınız <strong>1 iş günü içinde</strong>
    ...
    console.log("[video-strateji] kullanici:", JSON.stringify(kullaniciResult));

    return NextResponse.json({
      success: true,
      message: `${sektor} sektörü için ${hedef} odaklı video strateji öneriniz 1 iş günü içinde e-posta adresinize gönderilecek.`,
    });
  } catch (err) {
    console.error("[video-strateji]", err);
